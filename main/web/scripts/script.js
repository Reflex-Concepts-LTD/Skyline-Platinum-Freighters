...
function initAnima(t) {
    !function (i) {
        ...
        "default" != s || isEmpty(m) || (s = m), isEmpty(c) && (c = 500), isEmpty(n) && (n = t), i(n).each(function (e) {
            if (!isEmpty(i(this).attr("data-anima")) && 0 === e)
                return n = t, !1
        ...
        i(n).each(function (m) {
            var $ = f;
            if (m === h - 1 && "desc" === l && ($ = 0), !i(this).hasClass("anima") && n != t && isEmpty(r))
                p = this;
                ...
                i(g).attr("aid", b), e.length > 30 && (e.shift(), a.shift()), e.push([b, setTimeout(function () {
                        ...
                        if (!isEmpty(i(g).attr("class")) && -1 != i(g).attr("class").indexOf("anima-"))
                            for (var e = i(g).attr("class").split(" "), n = 0; n < e.length; n++)
                                -1 != e[n].indexOf("anima-") && (t = e[n].replace("anima-", ""));
            ...
            i(n).each(function () {
                ...
                if (!isEmpty(r)) {
                    for (m = 0; m < e.length; m++)
                        e[m][0] == r && clearTimeout(e[m][1]);
                    for (m = 0; m < a.length; m++)
                        a[m][0] == r && clearTimeout(a[m][1])
                }
                    ...
                    if (null != d && !i.contains(d, this) || null == d) {
                        ...
                        try {
                            if (-1 != i(this).attr("class").indexOf("anima-"))
                                for (var h = i(this).attr("class").split(" "), m = 0; m < h.length; m++)
                                    -1 != h[m].indexOf("anima-") && (c = h[m].replace("anima-", ""))
                        } catch (t) {
                        }
...
function onePageScroll(t) {
    isEmpty(t) || jQuery(t).find('a[href ^= "#"]').on("click", function (t) {
        ...
        if (e.length > 0) {
            ...
            "#" != i && i.length > 2 && jQuery(this).closest("header").length && (document.location = window.location.protocol + "//" + window.location.host)
    })
}
...
window.onload = function () {
    ...
    for (t in i)
        i[t].hasAttribute && i[t].hasAttribute("data-include") && function (t, i) {
            var e = /^(?:file):/,
            ...
            !function (t) {
                ...
                if ($.isFunction($.fn.imagesLoaded)) {
                    ...
                    $(this).hasClass("maso-box") && (i = !0, t = this), e.on("progress", function (e, a) {
                        ...
                        if (isEmpty(o.get(0)) || (c = o.get(0).clientWidth, d = o.get(0).clientHeight), "loaded" == s) {
                            ...
                            if (r > d)
                                $(a.img).css("margin-top", "-" + Math.floor((r - d) / 2) + "px");
                            else {
                                var m = l * (d / r);
                        ...
                        if (!isEmpty(t) && t.length > 10) {
                            ...
                            if (i.length > 0) {
                                for (var e = $(".shop-menu-cnt"), a = $(e).find(".cart-total").attr("data-currency"), s = 0, n = "", o = 0; o < i.length; o++)
                                    s += i[o].price * i[o].quantity, n += "<li onclick=\"document.location = '" + i[o].link + '\'" class="cart-item"><img src="' + i[o].image + '" alt=""><div class="cart-content"><h5>' + i[o].title + '</h5><span class="cart-quantity">' + i[o].quantity + " x " + a + i[o].price + "</span></div></li>";
                ...
                $(window).scroll(function () {
                    ...
                    if (isEmpty(i) || (i = "#" + i + " "), !isEmpty(t))
                        if (t.indexOf("list") > -1)
                            $(i + ".grid-box .grid-item:nth-child(" + t.replace("list-", "") + ") .img-box").click(), $(i + ".maso-box .maso-item:nth-child(" + t.replace("list-", "") + ") .img-box").click();
                        else if (t.indexOf("slide") > -1)
                            $(i + ".slides > li:nth-child(" + t.replace("slide-", "") + ") .img-box").click();
                        else {
                            var e = $("#" + t);
                            if ($(e).length)
